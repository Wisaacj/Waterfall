...
class ForwardRateCurve:
    ...
    def __init__(self, dates: list[date], rates: list[float]):
        ...
        """
        assert len(dates) == len(rates), "Dates and rates must have the same length."
        ...
        self.interpolator = interpolate.interp1d(
            [d.toordinal() for d in self.dates],
            rates,
            kind='cubic', # Use cubic splines to interpolate between points.
            bounds_error=False, # Do not raise an error if the interpolation is out of bounds.
            fill_value=(rates[0], rates[-1]) # Use the first or last known rate if the interpolation is out of bounds.
        )
